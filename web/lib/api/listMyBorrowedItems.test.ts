...
import { listMyBorrowedItems } from "./listMyBorrowedItems"
...
test("will call the list-my-borrowed-items route with the given input", async () => {
    ...
    const result = await listMyBorrowedItems("token", { borrower: "a@b.com" })

    expect(result).toEqual({ items: [], nextPageToken: undefined })
    const [url] = mockFetch.mock.calls[0]
    expect(url.toString()).toContain("list-my-borrowed-items")
})
