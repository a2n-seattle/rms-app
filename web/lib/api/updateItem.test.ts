...
test("will call the update-item route with the given input", async () => {
    ...
    const result = await updateItem("token", { id: "family-123", name: "New Name" })

    expect(result).toEqual("family-123")
    const [url] = mockFetch.mock.calls[0]
    expect(url.toString()).toContain("update-item")
})
