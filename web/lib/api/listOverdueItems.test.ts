...
import { listOverdueItems } from "./listOverdueItems"
...
test("will call the list-overdue-items route with the given input", async () => {
    ...
    const result = await listOverdueItems("token", { borrower: "a@b.com" })

    expect(result).toEqual({ items: [], nextPageToken: undefined })
    const [url] = mockFetch.mock.calls[0]
    expect(url.toString()).toContain("list-overdue-items")
})
