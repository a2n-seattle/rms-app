...
test("will call the list-history route with the given input", async () => {
    ...
    const result = await listHistory("token", { borrower: "a@b.com" })

    expect(result).toEqual({ items: [], nextPageToken: undefined })
    const [url] = mockFetch.mock.calls[0]
    expect(url.toString()).toContain("list-history")
})
